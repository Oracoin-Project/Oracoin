...
{
    // Convert oracoin:// to oracoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("oracoin://"))
    {
        uri.replace(0, 12, "oracoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OraCoin.lnk";
}
...
{
    return GetAutostartDir() / "oracoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OraCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  oracoin-qt [" + tr("command-line options") + "]                     " + "\n";
